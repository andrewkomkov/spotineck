...
| **shairport** | AirPlay 1 приёмник ([shairport-sync](https://github.com/mikebrady/shairport-sync) 5.0.4 classic, свой образ `andrewkomkov/spotineck-shairport`) → пишет в `/music/airplay`. mDNS публикует через единый avahi из owntone (см. ниже). |
...
- `andrewkomkov/spotineck-shairport:5.0.4` — Shairport Sync 5.0.4 classic (AirPlay 1) → pipe, multi-stage из исходников.

### mDNS: один avahi на host-сеть

`network_mode: host` означает один общий сетевой неймспейс. На нём должен быть
**ровно один** `avahi-daemon`. У Linux-хоста он обычно уже запущен, поэтому оба
контейнера (`owntone` и `shairport`) используют **avahi хоста** через его system
D-Bus (`bind /run/dbus`) и свой avahi **не поднимают**. Если этого не сделать, в
host-сети оказывается несколько avahi (хостовый + по одному в каждом контейнере),
они дерутся за имя хоста (`nettop` → `nettop-2` → … → `nettop-298`), записи
`_raop._tcp`/`_daap._tcp` «прыгают», и на iPhone/iPad это ровно «**не удаётся
подключиться к spotineck**». Аналогично librespot обходится своим `libmdns`.

> Если на хосте avahi нет — entrypoint `owntone` поднимет собственный dbus+avahi
> как фолбэк (тогда `shairport` отключать не нужно, но проверь, что хостового avahi
> действительно нет, иначе вернётся конфликт).
...
shairport/    свой образ Shairport Sync 5.0.4 classic (Dockerfile + entrypoint)
